...
| `search_library` | Native full-text search with any/all/exact, exclude, category, century filters, pagination |
...
| `get_hadith_by_number` | **Hadith number → matn/isnad**, via shamela's own `رقم الحديث` lookup, verified on the page — no prebuilt index required |
| `get_tafsir_by_ayah` | **`surah:ayah` → tafsir passage**, via TOC + Qur'anic-bracket markers `﴿…(n)…﴾` |
...
src/
  ...
  lib/
    ...
    citation-detect.mjs     hadith markers («N -», «N - (M)»), ayah refs, ﴿…(n)…﴾ markers, surah headings
    hadith-index.mjs        static index + live resolvers (specialnumber2id / TOC walk)
  ...
  data/
    ...
    hadith-index.mjs        generated static index (optional cache; empty seed)
scripts/
  resolve-canonical-editions.mjs   re-check the whitelist against live shamela (--list <title> to explore)
  build-hadith-index.mjs           build the static index via specialnumber2id + on-page verification
  check-coverage.mjs               probe for absent subcontinental works → reports/
test/                              78 offline tests (node --test), incl. real-page fixture
.github/workflows/
...
node scripts/build-hadith-index.mjs --book 1681 --step=50 --dry-run   # smoke-run the index builder
